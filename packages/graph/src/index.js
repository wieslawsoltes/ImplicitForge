import { uid, clone, validateDomain } from '../../kernel/src/index.js';
...
export function topologicalOrder(doc, root = doc.root, strict = true) {
...
export function connect(doc, targetId, port, sourceId) {
...
export function downstream(doc, id) { const set = new Set([id]); let changed = true; while (changed) {
    changed = false;
    for (const n of doc.nodes)
...
export function graphDiagnostics(doc) { return doc.nodes.flatMap(n => { try {
